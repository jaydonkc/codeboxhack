...
test('inserts at every position with comparison answers consistent with the desired order', () => {
  ...
  for (let desiredIndex = 0; desiredIndex <= preferences.length; desiredIndex += 1) {
    ...
    while (session.status === 'active') {
      ...
      session = answerRanking(session, desiredIndex <= opponentIndex ? 'new' : 'existing');
    }
    assert.equal(session.status, 'placed');
    const expected = preferences.map(({ id }) => id);
    expected.splice(desiredIndex, 0, 'new');
    const actual = finishRanking(session, preferences).sort((a, b) => (a.rank ?? 0) - (b.rank ?? 0));
    assert.deepEqual(actual.map(({ id }) => id), expected);
    assert.deepEqual(actual.map(({ rank }) => rank), expected.map((_, index) => index + 1));
  }
});

test('first experience is placed without comparisons; pending entries are not opponents', () => {
  const preferences: Preference[] = [{ id: 'pending', band: 'liked', rank: null }];
...
test('ties join a group and shift following occupied ranks', () => {
  ...
  assert.deepEqual(Object.fromEntries(result.map(({ id, rank }) => [id, rank])), { a: 1, b: 2, c: 4, x: 2 });
  assert.deepEqual(beginRanking('y', 'liked', result).groups, [['a'], ['b', 'x'], ['c']]);
...
test('skipping the sole opponent and finishing an active session both save pending', () => {
  ...
  assert.equal(finishRanking(active, preferences).find(({ id }) => id === 'x')?.rank, null);
  const pending = answerRanking(active, 'skip');
  assert.equal(pending.status, 'pending');
  assert.equal(pending.lo, 0);
  assert.equal(pending.hi, 1);
  assert.equal(finishRanking(pending, preferences).find(({ id }) => id === 'x')?.rank, null);
});
...
test('reranking excludes self, preserves personal metadata, and keeps one vote per experience', () => {
  ...
  assert.deepEqual(result.find(({ id }) => id === 'a'), { ...preferences[0], rank: 2 });
  assert.deepEqual(result.find(({ id }) => id === 'other'), preferences[2]);
...
test('changing bands moves the same experience and preserves other band records', () => {
  ...
  assert.deepEqual(result.find(({ id }) => id === 'a'), { id: 'a', band: 'okay', rank: 1 });
  assert.deepEqual(result.find(({ id }) => id === 'b'), preferences[1]);
  assert.deepEqual(result.find(({ id }) => id === 'disliked'), preferences[3]);
});

test('duplicate persisted IDs cannot create extra opponents or votes', () => {
  const preferences = [...liked('a', 'b'), { id: 'a', band: 'liked', rank: 1 } as Preference];
  const session = beginRanking('x', 'liked', preferences);
  assert.deepEqual(session.groups, [['a'], ['b']]);
  assert.deepEqual(scorePreferences(preferences), scorePreferences(liked('a', 'b')));
...
test('band midpoints use decimal half-up rounding, including liked singleton 8.4', () => {
  ...
  assert.deepEqual(scorePreferences(preferences), { liked: 8.4, okay: 5, disliked: 1.7 });
});
...
test('scoring counts experiences, averages occupied tie ranks, and ignores pending entries', () => {
  ...
  assert.deepEqual(scorePreferences(preferences), { a: 8.8, b: 8.8, c: 7.5, pending: null });
  assert.deepEqual(scorePreferences([...preferences].reverse()), scorePreferences(preferences));
  assert.equal(scorePreferences(preferences.filter(({ id }) => id !== 'pending')).a, 8.8);
});
...
test('rank gaps are normalized and all-tied groups receive their band midpoint', () => {
  ...
  assert.deepEqual(scorePreferences([{ ...preferences[0], rank: 50 }]), { a: 8.4 });
});

test('scores stay in their band, remain ordered, and do not depend on other bands', () => {
  const bounds = { liked: [6.7, 10], okay: [3.4, 6.6], disliked: [0, 3.3] } as const;
  for (const band of ['liked', 'okay', 'disliked'] as const) {
    const preferences: Preference[] = Array.from({ length: 40 }, (_, index) => ({ id: `${band}-${index}`, band, rank: index + 1 }));
    const scores = scorePreferences(preferences);
    const values = preferences.map(({ id }) => scores[id] as number);
    assert.ok(values.every((value) => value >= bounds[band][0] && value <= bounds[band][1]));
    assert.ok(values.every((value, index) => index === 0 || value <= values[index - 1]));
    const unrelated: Preference = { id: 'other', band: band === 'liked' ? 'okay' : 'liked', rank: 1 };
    const expanded = scorePreferences([...preferences, unrelated]);
    assert.deepEqual(preferences.map(({ id }) => expanded[id]), values);
  }
});
...
test('display filters only select from global scores and never rescore visible subsets', () => {
  ...
  const visibleScores = Object.fromEntries(displayed.map((id) => [id, scores[id]]));
...
test('five-question cap leaves unresolved insertion pending and refuses further answers', () => {
  const preferences = liked(...Array.from({ length: 64 }, (_, index) => `item-${index}`));
  let session = beginRanking('x', 'liked', preferences);
  ...
  assert.equal(finishRanking(session, preferences).find(({ id }) => id === 'x')?.rank, null);
});

test('a decisive answer on the fifth question resolves, and skips also consume the cap', () => {
  const preferences = liked(...Array.from({ length: 31 }, (_, index) => `item-${index}`));
  let placed = beginRanking('x', 'liked', preferences);
  for (let index = 0; index < 5; index += 1) placed = answerRanking(placed, 'new');
  assert.equal(placed.compared, 5);
  assert.equal(placed.status, 'placed');
  assert.equal(finishRanking(placed, preferences).find(({ id }) => id === 'x')?.rank, 1);
...
test('invalid placed ranks are treated as pending', () => {
  ...
  assert.deepEqual(scorePreferences(preferences), { nan: null, zero: null, infinite: null });
