...
function uniquePreferences(preferences: readonly Preference[]): Preference[] {
  return [...new Map(preferences.map((preference) => [preference.id, preference])).values()];
}

function isPlaced(preference: Preference): boolean {
  return preference.rank !== null && Number.isFinite(preference.rank) && preference.rank >= 1;
}

function orderedGroups(preferences: readonly Preference[], band: Band): string[][] {
...
function opponentIndex(session: RankingSession): number | null {
  if (session.status !== 'active' || session.compared >= MAX_RANKING_QUESTIONS) return null;
  ...
  for (let index = session.lo; index < session.hi; index += 1) {
    ...
    if (best === null || Math.abs(index - midpoint) < Math.abs(best - midpoint)) best = index;
  }
  return best;
}
...
function settleSession(session: RankingSession): RankingSession {
  ...
  if (session.lo === session.hi) {
    return { ...session, status: 'placed', placement: { kind: 'insert', index: session.lo } };
  }
  ...
  if (opponentIndex(session) === null) {
    return { ...session, status: 'pending', pendingReason: 'skipped-opponents' };
  }
  return session;
}
...
export function beginRanking(
  ...
  const groups = orderedGroups(preferences.filter((preference) => preference.id !== id), band);
 ...
 */
export function answerRanking(session: RankingSession, answer: RankingAnswer): RankingSession {
  const index = opponentIndex(session);
...
export function finishRanking(
  ...
  const result = uniquePreferences(preferences).map((preference) => {
    if (preference.id === session.id) {
      found = true;
      return { ...preference, band: session.band, rank: ranks.get(session.id) ?? null };
    }
  ...
  if (!found) result.push({ id: session.id, band: session.band, rank: ranks.get(session.id) ?? null });
  return result;
}
 ...
 */
export function scorePreferences(preferences: readonly Preference[]): Record<string, number | null> {
  const unique = uniquePreferences(preferences);
  const scores: Record<string, number | null> = Object.fromEntries(unique.map(({ id }) => [id, null]));
  for (const band of ['disliked', 'okay', 'liked'] as const) {
    ...
    for (const group of groups) {
      ...
      const score = Math.floor((2 * scaledNumerator + denominator) / (2 * denominator)) / 10;
